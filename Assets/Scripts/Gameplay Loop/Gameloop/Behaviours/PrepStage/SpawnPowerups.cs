...
namespace TrojanMouse.GameplayLoop{
    public class SpawnPowerups : GLNode{
        ...
        bool hasSpawned;
        public SpawnPowerups(GameLoopBT gameloop, GameObject powerupObj, Level.PowerupsToBeDispenced[] powerups, Transform powerupStorage){
        ...
        public override NodeState Evaluate(){
            ...
            for (int i = 0; i < nanaBetsys.Length; i++){
                GameObject powerup = gameloop.SpawnObj(powerupObj, Vector3.zero, Quaternion.identity, storage);
                powerup.GetComponent<Powerup>().UpdateType(nanaBetsys[i].powerupType);
